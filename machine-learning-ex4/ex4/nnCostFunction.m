...
for i = 1:rows(X)
    ...
    z3 = Theta2*[1; a2];
    hArr = arrayfun(@sigmoid, z3);
    
    y_t = y_i(:, y(i, 1));
    J += ((-y_t')*log(hArr) - (1 - y_t')*log(1 .- hArr));
...
t1_sq = Theta1.^2;
t2_sq = Theta2.^2;
reg = (sum(t1_sq(:, 2:end)(:)) + sum(t2_sq(:, 2:end)(:)))*(lambda/(2*m));
...
if(lambda != 0) 
    % lambda*[zeros(size(Theta2, 1), 1) Theta2(:, 2:end)](:, 1:3)
    % (Delta2/m)(:, 1:3)
    % Theta2_grad(:, 1:3)
    % lambda*[zeros(size(Theta1, 1), 1) Theta1(:, 2:end)](:, 1:3)
    % (Delta1/m)(:, 1:3)
    % Theta1_grad(:, 1:3)
end 
